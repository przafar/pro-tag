<template>
  <div class="w-full back-color">
    <div class="container mx-auto text-center pt-10 px-6 pb-64">
      ...
      <div class="grid grid-cols-6 gap-2 lg:h-48 xl:h-48 bg-white rounded-lg py-4">
          ...
          <div class="col-span-4  mr-2">
            <input v-model="name" placeholder="Ваше имя" type="text" class="placeholder-white shadow-inner xl:mr-4 lg:mr-4 mr-0 focus:placeholder-gray-500  w-full xl:w-64 lg:w-64 lg:mt-2 xl:mt-2 focus:outline-none text-white focus:shadow-inner focus:text-gray-900 focus:bg-white border-b-2 focus:border-gray-500 bg-gray-600 h-10 px-4 rounded-full">
            <input v-model="userInfo" placeholder="О вас" type="text" class="placeholder-white shadow-inner focus:placeholder-gray-500  w-full xl:w-64 lg:w-64 lg:mt-2 xl:mt-2 focus:outline-none mt-2 text-white focus:shadow-inner focus:text-gray-900 focus:bg-white border-b-2 focus:border-gray-500 bg-gray-600 h-10 px-4 rounded-full">
      ...
      <label for="file-input" class="focus:outline-none mx-auto block text-center pt-3 bg-green-600 h-12 w-48 rounded-full mt-8 mb-8 text-white">Добавить фото</label>
      <input type="file" id="file-input" class="img-input" @change="onFileSelected">
      <button v-if="this.info.profile.name.length < 3" @click.prevent="nextPage" @click="onUpload" class="mx-auto block focus:outline-none bg-blue-500 h-12 w-48 rounded-full mt-4 mb-8 text-white">Дальше</button>
      <button v-else @click.prevent="saveInfo" @click="onUpload" class="mx-auto block focus:outline-none bg-blue-500 h-12 w-48 rounded-full mt-4 mb-8 text-white">Сохранить</button>
